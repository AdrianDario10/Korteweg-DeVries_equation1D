...
if __name__ == '__main__':
    ...
    tx_bnd_down[...,1] = -1    # x_bnd = -1

    # create training output
    u_eqn = np.zeros((num_train_samples, 1))               # u_eqn = 0
    u_ini = u_ini = u0(tf.constant(tx_ini)).numpy() 
    ...
    t_flat = np.linspace(0, 1, 201)
    x_flat = np.linspace(-1, 1, 512)
    ...
    cbar.mappable.set_clim(-1,np.max(np.max(u)))
    ...
    # Solution at time 0, 0.2 and 0.8
    # predict u(t,x) distribution
    ...
    u_ = network.predict(tx, batch_size=num_test_samples)
    ax2.plot(x_flat, u_)
    ax2.set_title('t={}'.format(0.2), fontdict = font1)
    ax2.set_xlabel('x', fontdict = font1)
    ax2.set_ylabel('u(t,x)', fontdict = font1)
    ax2.tick_params(labelsize=15)

    tx = np.stack([np.full(t_flat.shape, 0.8), x_flat], axis=-1)
    u_ = network.predict(tx, batch_size=num_test_samples)
    ax3.plot(x_flat, u_)
    ax3.set_title('t={}'.format(0.8), fontdict = font1)
